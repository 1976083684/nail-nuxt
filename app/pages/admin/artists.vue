...
const form = ref<any>({
  name: '', title: '', phone: '', rating: 5, experience_years: 0,
...
function openNew() {
  editId.value = null
  form.value = { name: '', title: '', phone: '', rating: 5, experience_years: 0, specialty: '', image_url: '', bio: '', certifications: [], works: [] }
  editing.value = true
}
...
function openEdit(item: any) {
  ...
  form.value = {
    name: item.name, title: item.title || '', phone: item.phone || '', rating: item.rating || 5,
      ...
      <div v-for="item in items" :key="item.id" class="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex items-start justify-between">
        <div class="flex items-start gap-4">
          ...
          <div>
            ...
            <p class="text-blue-500 text-sm">{{ item.title }}</p>
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model="form.phone" maxlength="11" class="w-full px-3 py-2 border rounded-lg text-sm focus:border-blue-400 outline-none" placeholder="选填" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">评分</label>
              <input v-model.number="form.rating" type="number" step="0.1" min="0" max="5" class="w-full px-3 py-2 border rounded-lg text-sm focus:border-blue-400 outline-none" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">从业年限</label>
              <input v-model.number="form.experience_years" type="number" class="w-full px-3 py-2 border rounded-lg text-sm focus:border-blue-400 outline-none" />
            </div>
          </div>
          ...
          <div>
            ...
            <input v-model="form.specialty" class="w-full px-3 py-2 border rounded-lg text-sm focus:border-blue-400 outline-none" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">简介</label>
            <textarea v-model="form.bio" rows="3" class="w-full px-3 py-2 border rounded-lg text-sm focus:border-blue-400 outline-none" />
          </div>

          <div>
            <div class="flex items-center justify-between mb-2">
              <label class="text-sm font-medium text-gray-700">资质证书</label>
              <button class="text-blue-500 text-sm hover:underline" @click="addCert"><i class="fas fa-plus mr-1" />添加</button>
            </div>
            <div v-for="(_, i) in form.certifications" :key="i" class="flex gap-2 mb-2">
              <input v-model="form.certifications[i]" class="flex-1 px-3 py-2 border rounded-lg text-sm focus:border-blue-400 outline-none" placeholder="证书名称" />
          ...
          <div>
            <div class="flex items-center justify-between mb-2">
              ...
              <button class="text-blue-500 text-sm hover:underline" @click="addWork"><i class="fas fa-plus mr-1" />添加</button>
            </div>
            <div v-for="(_, i) in form.works" :key="i" class="mb-3 p-3 border border-gray-100 rounded-lg">
              ...
              <input v-model="form.works[i].description" class="w-full mt-2 px-3 py-2 border rounded-lg text-sm focus:border-blue-400 outline-none" placeholder="作品描述（选填）" />
            </div>
          </div>
        </div>
